;   Muvee Custom Style by SoftSteph ;) -  V13
...
(min-segment-duration-for-transition 0.25)

(preferred-transition-duration 0.75)
  ...
  (layers (A B)
    ...
    (effect-stack
      ...
      (effect "Alpha" ()
              (input 0 A)
              (param "Alpha" 1.0
                     (bezier 1.0 0.0 1.0 3.0))))
    ; fade in input B
    (effect "Alpha" ()
            (input 0 B)
            (param "Alpha" 0.0
                   (bezier 1.0 1.0 3.0 1.0)))))
...
(Captions.set-default-font-properties 0xffffffff 24 "Sansation")
...
(if (= TEXT_SLIDE 'off)
   ...
   (Captions.presenter (fn args
	...
	;Si ON - Translation
   (Captions.presenter (fn args
                      (Captions.Slide.direction (random-direction))
                      (apply Captions.Slide args))))

(Captions.text-properties (fn (info . _)
                            (put info.LayoutX0 0.05)
                            (put info.LayoutY0 0.05)
                            (put info.LayoutX1 0.95)
                            (put info.LayoutY1 0.95)))						
...
(define FOREGROUND_FX
  (effect-stack
    scanlines-fx
	(cg:color-process-fx SATURATION COLOR_EFFECT COLOR_STRENGTH)
    (effect "Perspective" (A))))
...
;;; transitions between credits and body ;;;

(define title/credits-tx-dur
  ; ranges from 0.5 to 1.0 seconds
  (+ (* (- 1.0 AVERAGE_SPEED) 0.5) 0.5))
